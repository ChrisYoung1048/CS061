...
PRINT_STRING_FORM
	...
	ADD R2, R2, #1 ; Increment Counters
...
; R2 will be incremeted to traverse through user inputted string
...
NEXT_CHARACTER ;WRITE A SUBROUTINE TO ACCOUNT FOR SCRAMBLING (COMPARE SUM OF CHARACTERS IN DECIMAL)
	; Check for end of instruction here
	; ===
	ADD R6, R6, #0 ; Set R6 to LMR
	BRz PRINT_USER_INSTRUCTION ; If the character is 0, then the instruction is found
	; ===
...
PRINT_USER_INSTRUCTION
	LDR R0, R2, #0
	BRz END_SUB_FO ; Zero denoting the end of the string
	OUT
	ADD R2, R2, #1
	BR PRINT_USER_INSTRUCTION
...
.STRINGZ "LD" 
.STRINGZ "ST" 
.STRINGZ "JSR" ; JSR and JSRR are both 0100
.STRINGZ "AND" 
.STRINGZ "LDR" 
.STRINGZ "STR" 
.STRINGZ "RTI" 
